...
class ProcessFrame(tk.Frame):
    ...
    def processShowPage(self):
        ...
        self.sheet_data = self.list.get("sheetname")
        ...
        self.address_show = tk.Frame(self.process_show)
        self.address_show.pack(side=TOP, anchor=tk.W, pady=10)
        ttk.Label(self.address_show, text="  工作表名称:").pack(side=tk.LEFT)
        ttk.Label(self.address_show, text=self.sheet_data).pack(side=tk.RIGHT)
        ...
        self.button_show.pack(side=BOTTOM, anchor=tk.S, pady=30)
    ...
    def readButton(self):
        Method.readMethod(self, self.list.get("start_path"), self.list.get("excel"), self.list.get("start"), self.list.get("sheetname"))
    ...
    def cpoyButton(self):
        ...
        sheet_data = openpyxl.load_workbook(excel_data)[self.sheet_data]
        # 循环遍历表格文件名
        for line in range(1, sheet_data.max_row + 1):
    ...
    def moveButton(self):
        # 打开表格
        sheet = openpyxl.load_workbook(self.excel_data)[self.sheet_data]
        # 循环遍历表格文件名
        for line in range(1, sheet.max_row + 1):
            print(line)
            file_name = sheet.cell(line, int(self.start_data)).value
            print(file_name)
            # 判断起始地址以及目标地址文件是否存在
    ...
    def renamingButton(self):
        Method.renamingMethod(self, self.start_path_data, self.excel_data, self.start_data, self.target_data, self.sheet_data)
    ...
    def pocessInputPage(self):
        ...
        self.address_show.pack(side=TOP, anchor=tk.W, pady=10)
        ttk.Label(self.address_show, text=" 工作表名称:").pack(side=tk.LEFT)
        ttk.Entry(self.address_show, text=self.sheet, width=80).pack(side=tk.RIGHT)
    ...
    def defineDataButton(self):
        if not os.path.exists(self.start_path.get()) \
        ...
        elif not self.sheet.get() in load_workbook(self.excel.get()).sheetnames:
            ...
            self.list['sheetname'] = self.sheet.get()
